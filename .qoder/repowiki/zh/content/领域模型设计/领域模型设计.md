...
- [WorkstationConfigMapper.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Infrastructure/Repositories/WorkstationConfigMapper.cs)
...
- Application：应用层，包含命令、处理器、事件、事件处理器、服务、DTO、验证器等
- Infrastructure：基础设施层，包含仓储实现、序列化转换器、通信驱动、后台服务等
- Api：应用服务入口，控制器、中间件、后台服务等
- Share：共享模块，异常等跨层共享能力
...
A1["命令与处理器<br/>SaveWorkstationConfigCommand*"]
A2["事件与处理器<br/>WorkstationConfigUpdatedEvent*"]
A3["服务<br/>CollectionTaskManager / DataCollectionAppService"]
A4["DTO 与验证器"]
A5["映射器"]
end
...
Cmd --> Hdl["命令处理器<br/>SaveWorkstationConfigCommandHandler"]
Hdl --> Agg["聚合根<br/>WorkstationConfig"]
...
Hdl --> Svc["应用服务<br/>DataCollectionAppService"]
Svc --> Tsk["任务管理<br/>CollectionTaskManager"]
Hdl --> Repo["仓储接口<br/>IWorkstationConfigRepository"]
Repo --> InfraRepo["仓储实现<br/>WorkstationConfigRepository"]
Hdl --> Ev["领域事件<br/>WorkstationConfigUpdatedEvent"]
Ev --> EH["事件处理器<br/>WorkstationConfigUpdatedEventHandler / ClearConfigCacheEventHandler"]
```
...
### 实体生命周期管理
- 创建
  - 通过控制器接收SaveWorkstationConfigRequest
  - 命令处理器创建WorkstationConfig聚合，填充ProtocolConfig/EquipmentConfig/ParameterConfig
  - 持久化仓储保存WorkstationConfigEntity（JSON内容）
- 更新
  - 修改配置后，发布WorkstationConfigUpdatedEvent
  - 事件处理器触发缓存清理与重新加载，保证配置生效
- 删除
...
- WorkstationConfigUpdatedEventHandler：接收事件后执行后续处理（如刷新缓存）
- ClearConfigCacheEventHandler：清理相关缓存键，确保读取最新配置
...
participant EH2 as "ClearConfigCacheEventHandler"
Cmd->>Bus : "发布 WorkstationConfigUpdatedEvent"
Bus-->>EH1 : "通知"
EH1-->>EH1 : "执行业务处理"
Bus-->>EH2 : "通知"
