...
- [Persistence/Repositories/WorkstationConfigRepository.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Infrastructure/Persistence/Repositories/WorkstationConfigRepository.cs)
- [Persistence/Repositories/WorkstationConfigPersistenceRepository.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Infrastructure/Persistence/Repositories/WorkstationConfigPersistenceRepository.cs)
- [Persistence/Repositories/EquipmentDataStorageRepository.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Infrastructure/Persistence/Repositories/EquipmentDataStorageRepository.cs)
...
## 更新摘要
**变更内容**
- 基础设施层架构：包含通信、数据处理、OPC UA集成、后台服务和持久化等核心功能
- 依赖注入配置：注册连接管理、协议驱动、数据处理、后台服务和仓储组件
- 持久化层：提供工作站配置仓储和设备数据存储仓储实现
- 后台服务职责：设备数据持久化后台服务和OPC UA托管服务
...
WCR["WorkstationConfigRepository<br/>工作站配置仓储"]
EDS["EquipmentDataStorageRepository<br/>设备数据存储仓储"]
...
- **持久化仓储**：工作站配置仓储（JSON解析与领域模型转换）、设备数据存储仓储
- **依赖注入**：集中注册HSL授权、连接管理器、后台服务、协议驱动、处理器、仓储与JSON选项
...
- **数据处理组件**：设备数据处理器、表达式转换器、虚拟点计算器均注册为单例
- **仓储服务**：工作站配置仓储注册为Scoped，设备数据存储仓储注册为Singleton
- **JSON选项**：注册多态转换器与命名策略
...
DI --> WCR["IWorkstationConfigRepository<br/>Scoped"]
DI --> WCPR["IWorkstationConfigPersistenceRepository<br/>Scoped"]
DI --> EDS["IEquipmentDataStorageRepository<br/>Singleton"]
...
## 结论
基础设施层通过清晰的抽象与模块化设计，实现了工业通信协议的可插拔扩展、稳定的连接管理、高效的数据处理与可靠的OPC UA发布。基础设施层提供了完整的服务架构，包含核心通信、数据处理、OPC UA集成和持久化功能。通过仓储接口实现领域模型与持久化存储的解耦，保持了良好的可维护性与可扩展性。
...
### 仓储服务使用指南
- **工作站配置仓储**：负责从数据库加载JSON配置并解析为领域模型
- **持久化仓储**：负责将工作站配置保存到数据库
- **设备数据存储仓储**：负责存储采集结果数据
