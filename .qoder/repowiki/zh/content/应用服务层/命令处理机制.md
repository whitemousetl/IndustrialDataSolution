...
EH1["ClearConfigCacheEventHandler"]
EH2["WorkstationConfigUpdatedEventHandler"]
end
subgraph "领域/基础设施"
...
Event --> |"Handle"| EH1
Event --> |"Handle"| EH2
EH2 --> |"ClearAllConnectionsAsync"| ConnMgr
EH2 --> |"StartOrRestartAllTasksAsync"| TaskMgr
EH2 --> |"StartOrRestartServerAsync"| PubMgr
EH1 --> |"Remove(CacheKeys)" --> Cache
Repo --> DB
```
...
## 核心组件
- 命令定义：SaveWorkstationConfigCommand承载请求载荷（WorkstationConfigDto），作为MediatR的IRequest载体。
...
- 事件与处理器：WorkstationConfigUpdatedEvent作为通知事件，ClearConfigCacheEventHandler清理缓存，WorkstationConfigUpdatedEventHandler负责连接、任务与发布服务器的重启。
...
participant EH1 as "ClearConfigCacheEventHandler"
participant EH2 as "WorkstationConfigUpdatedEventHandler"
...
Bus-->>EH2 : "Handle(event)"
EH2-->>EH2 : "清理连接/重启任务/重启发布服务器"
EH1-->>EH1 : "移除缓存键"
Handler-->>Mediator : "完成"
Mediator-->>API : "完成"
API-->>Client : "200 OK"
...
- 缓存清理：ClearConfigCacheEventHandler移除内存缓存键，确保后续读取使用最新配置。
...
- 命令定义：命令类文件路径参见[SaveWorkstationConfigCommand.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Application/Commands/SaveWorkstationConfigCommand.cs#L1-L9)。
- 处理器实现：处理器类文件路径参见[SaveWorkstationConfigCommandHandler.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Application/CommandHandlers/SaveWorkstationConfigCommandHandler.cs#L1-L32)。
...
- 事件与处理器：事件与处理器文件路径分别参见[WorkstationConfigUpdatedEvent.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Application/Events/WorkstationConfigUpdatedEvent.cs#L1-L11)、[WorkstationConfigUpdatedEventHandler.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Application/EventHandlers/WorkstationConfigUpdatedEventHandler.cs#L1-L40)、[ClearConfigCacheEventHandler.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Application/EventHandlers/ClearConfigCacheEventHandler.cs#L1-L26)。
- 依赖注入：扩展方法文件路径参见[DependencyInjection.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Application/DependencyInjection.cs#L1-L40)。
...
Event --> EH1["ClearConfigCacheEventHandler"]
Event --> EH2["WorkstationConfigUpdatedEventHandler"]
EH2 --> ConnMgr["IConnectionManager"]
EH2 --> TaskMgr["ICollectionTaskManager"]
EH2 --> PubMgr["IDataPublishServerManager"]
...
- 缓存未清理：核对ClearConfigCacheEventHandler是否收到事件并执行缓存移除。
