...
- 域测试：Domain（领域模型与值对象）
- 基础设施测试：Infrastructure.Test（集成测试）
- API测试：Api.Test（集成测试）
...
- [IndustrialDataProcessor.Domain\Entities\WorkstationConfigEntity.cs](file://IndustrialDataProcessor.Domain/Entities/WorkstationConfigEntity.cs#L1-L7)
...
## 架构总览
单元测试在测试金字塔中承担“最底层”的快速反馈角色，确保验证器与应用服务的行为稳定可靠，减少回归风险。
...
UT->>CMD : "Handle(SaveWorkstationConfigCommand)"
CMD->>CMD : "dto.ToDomain()"
CMD->>CMD : "JsonSerializer.Serialize(domain)"
CMD->>REPO : "AddAsync(entity)"
REPO-->>CMD : "完成"
CMD->>MED : "Publish(WorkstationConfigUpdatedEvent)"
MED-->>CMD : "完成"
CMD-->>UT : "完成"
```
...
- [IndustrialDataProcessor.Application.Test\Services\WorkstationConfigServiceTests.cs](file://IndustrialDataProcessor.Application.Test/Services/WorkstationConfigServiceTests.cs#L207-L235)
...
## 结论
本项目的单元测试围绕验证器与应用服务展开，通过Mock隔离外部依赖，覆盖工作站、协议、设备与参数的完整验证逻辑与边界条件。配合清晰的断言策略与测试数据准备方法，能够有效保障配置验证的正确性与稳定性。建议持续完善覆盖率与质量度量，确保关键路径与异常分支均得到充分验证。
