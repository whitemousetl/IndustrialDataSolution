...
public class EquipmentDataHostingService(DataCollectionChannel channel, IEquipmentDataProcessor equipmentDataProcessor, IEquipmentDataStorageRepository equipmentDataStorageRepository, ILogger<EquipmentDataHostingService> logger) : BackgroundService
{
    ...
    protected async override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            await foreach (var (_, _, JsonMap) in _channel.DbChannel.ReadAllAsync(stoppingToken))
            {
                try
                {
