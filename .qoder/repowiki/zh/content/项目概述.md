...
subgraph "基础设施层"
INFRA["IndustrialDataProcessor.Infrastructure<br/>通信驱动/连接管理/OPC UA/仓储实现"]
PERSIST["IndustrialDataProcessor.Infrastructure.Persistence.SqlSugar<br/>仓储实现+PO映射"]
end
subgraph "共享与仿真"
SHARE["IndustrialDataProcessor.Share<br/>共享异常与通用能力"]
...
- 持久化层（Persistence.SqlSugar）
  - 提供工作站配置与设备数据的仓储实现，支撑配置与历史数据存储。
...
APP --> INFRA["基础设施层<br/>驱动/连接/OPC UA/后台服务"]
...
- 依赖注入与生命周期
  ...
  - 基础设施层：连接管理器、驱动、OPC UA托管服务注册为单例或后台服务。
...
- 技术栈概览
  ...
  - 依赖注入：标准DI容器
- 工业通信协议支持（示例）
  - TCP类：Modbus（RTU/TCP）、西门子S7、欧姆龙（FINS/UDP/TCP）、汇川/台达等
  - 特殊协议：IEC 61850-90-5（IEC104）、OPC UA（客户端）
