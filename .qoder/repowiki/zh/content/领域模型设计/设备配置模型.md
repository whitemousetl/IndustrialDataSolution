...
## 更新摘要
**变更内容**
- 新增设备参数管理功能：AddParameter和RemoveParameter方法
- 增强参数验证规则：重复参数标签验证和空参数地址验证
- 完善参数生命周期管理：支持动态添加和移除参数配置
- 强化数据一致性保证：在领域层实现参数管理逻辑
...
- [ParameterConfig.cs](file://IndustrialDataProcessor.Domain/Workstation/Configs/ParameterConfig.cs#L1-L121)
...
- 设备配置实体（EquipmentConfig）
  ...
  - **新增功能**：参数管理能力（AddParameter、RemoveParameter、GetMonitorParameters）
...
- 设计原则
  ...
  - **新增职责**：参数生命周期管理，确保参数完整性与一致性
- 业务价值
  - 统一设备维度的采集策略与命名规范
  - 支持按设备进行启停与批量配置管理
  - **新增**：动态参数管理能力，支持运行时参数调整
- 关键关系
...
- **新增参数管理验证**
  - 参数标签（Label）必填且唯一，防止重复配置
  - 参数地址（Address）必填，确保采集点有效性
  - 参数验证在领域层实现，保证数据一致性
- 参数级验证（通过参数验证器）
...
ParamValidator --> DomainValidation["领域层参数管理验证"]
DomainValidation --> Done(["验证完成"])
```

**图表来源**
- [EquipmentConfigDtoValidator.cs](file://IndustrialDataProcessor.Application/Validators/EquipmentConfigDtoValidator.cs#L1-L43)
- [EquipmentConfigDto.cs](file://IndustrialDataProcessor.Application/Dtos/WorkstationDto/EquipmentConfigDto.cs#L1-L39)
- [EquipmentConfig.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Domain/Workstation/Configs/EquipmentConfig.cs#L70-L87)
- [ProtocolType.cs](file://IndustrialDataProcessor.Domain/Enums/ProtocolType.cs#L1-L231)
...
- **AddParameter 方法**
  - 功能：向设备添加新的参数配置
  - 参数验证：确保参数对象不为空、标签不为空、标签唯一性
  - 异常处理：重复标签抛出领域异常
  - 返回：void（成功时）
- **RemoveParameter 方法**
  - 功能：根据标签移除设备参数
  - 参数验证：标签字符串验证
  - 返回：bool（移除成功返回true，否则false）
- **GetMonitorParameters 方法**
  - 功能：获取所有监控参数
  - 返回：IEnumerable<ParameterConfig>（监控状态为true的参数集合）

```mermaid
flowchart TD
AddParam["AddParameter(parameter)"] --> NullCheck["检查参数是否为空"]
NullCheck --> InitList["初始化参数列表"]
InitList --> LabelCheck["检查标签是否为空"]
LabelCheck --> DuplicateCheck["检查标签是否重复"]
DuplicateCheck --> AddToList["添加到参数列表"]
AddToList --> Success["添加成功"]
RemoveParam["RemoveParameter(label)"] --> ListCheck["检查参数列表是否存在"]
ListCheck --> FindParam["查找指定标签的参数"]
FindParam --> RemoveCheck["检查是否找到参数"]
RemoveCheck --> RemoveFromList["从列表中移除参数"]
RemoveFromList --> ReturnBool["返回移除结果"]
```

**图表来源**
- [EquipmentConfig.cs](file://IndustrialDataSolution/IndustrialDataProcessor.Domain/Workstation/Configs/EquipmentConfig.cs#L70-L97)
...
- **新增参数管理性能**
  - 参数添加/移除操作时间复杂度为O(n)，其中n为参数数量
  - 建议批量操作参数以提高性能
  - 监控参数查询使用LINQ过滤，注意大数据量时的性能影响
...
## 结论
设备配置模型以领域驱动为核心，通过清晰的聚合边界与严格的验证约束，实现了设备、变量与协议之间的强关联与可维护性。配合工作站配置与命令处理器，系统在创建、修改、删除等全生命周期中保持数据一致性与可观测性。**新增的参数管理功能进一步增强了系统的灵活性，支持动态参数调整，满足不同工业场景下的实时配置需求。**在不同工业场景中，合理选择设备类型与仪表类型，并遵循最佳实践与常见配置模式，可显著提升采集效率与数据质量。
...
- **新增参数管理最佳实践**
  - 参数标签命名规范：使用有意义的英文标识符，避免特殊字符
  - 参数地址规范：遵循协议地址约定，确保地址唯一性
  - 参数验证策略：在添加参数前进行预验证，减少运行时错误
  - 参数生命周期管理：定期审查参数配置，清理无效参数
